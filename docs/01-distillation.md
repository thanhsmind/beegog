...
- `history/<feature>/CONTEXT.md` as locked source of truth; `history/learnings/critical-patterns.md` as mandatory pre-planning context.
...
- **File-based communication:** workers and reviewers write markdown reports to `history/<feature>/reports/` with deterministic names; the orchestrator reads files, not memories. Works offline, diffable, no MCP required.
