...
4. **A state layer next to the logs (decision 0001).** Every upstream memory mechanism is history-shaped — append-only decisions, dated learnings, per-feature docs. bee adds the complementary state-shaped artifacts: `docs/specs/<area>.md` (the *current* behavior and requirements of a long-lived area, overwritten to match reality) and `docs/specs/reading-map.md` (what lives where). Compounding writes them by merging `behavior_change` cell deltas at feature close; the hive scout reads the touched area's spec before its code; grooming's entropy score measures spec rot (`stale specs` term). Log answers "how did we get here"; spec answers "where are we" — a fresh session reads spec → decisions → history, in that order. Full design: [02-architecture.md](02-architecture.md) §state layer and [decisions/0001-state-layer.md](decisions/0001-state-layer.md).
