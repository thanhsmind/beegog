...
- **Confidence:** 0.75 — **first dogfood DONE, 2026-07-14 (cell `ao-2e`, decision AO20).** The advice/read path ran end to end for the first time: `codex exec -m gpt-5.6-sol -s read-only -c model_reasoning_effort="high" -`, prompt on stdin, correct answer against a real repo question, exit 0. It needed **no `--yolo` and no write access**. `agy --sandbox --mode plan` (no bypass flag) did the same.
  **Why it took four days and nobody noticed:** the shipped command was **malformed**. `workspace-write` sat in the `[PROMPT]` positional (per `codex exec --help`, the bare positional *is* the prompt), `-s` was missing, and `--yolo` (= `--dangerously-bypass-approvals-and-sandbox`) meant **no sandbox ever ran**. The advisor had been receiving the literal string `workspace-write` as its question. A broken advisor reports nothing — advice is prose, and prose always looks like an answer.
  **Still NOT proven, and explicitly still pending:** the **cell-execution** path. Measured 2026-07-14: an external CLI's cwd is **not** the repo root (`agy` relocates to its own sandbox), so the `.bee/bin` helpers invoked by relative path run against a **phantom `.bee/`** — reservations, caps and verify results land where bee never reads, while the worker reports done. Absolute paths fix it; nothing has proven that end to end yet. **Do not route a cell to a cli tier on the strength of this dogfood.**
