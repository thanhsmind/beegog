...
The 5 runtime-sensitive skills (`bee-hive`, `bee-swarming`, `bee-executing`, `bee-validating`, `bee-reviewing`) are **one human-edited source tree**, not a fork per runtime. A skill file may fence a runtime-only passage with a strict, full-line HTML-comment marker pair:
