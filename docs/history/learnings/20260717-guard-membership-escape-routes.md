...
## Addendum 4 — Feature close (advisor-and-orchestration, same day)

**The feature's own mechanism gated its own last slice — and it worked.** Slice 5's Gate 3 was the first real pass through the AO3/AO13 precondition Slice 4 shipped: the approval refused until a genuine consult ran through the configured external adviser (read-only, evidence bundle on stdin), and the adviser's one substantive note (distinguish worker classes by authority and state effects, not dispatch mechanism) landed verbatim in the shipped contract text. Two durable observations: (1) a mechanical precondition composes cleanly with total-autopilot — bypass lifted no part of it and nothing needed a human; (2) the consult's marginal cost was one external call, and it materially improved the artifact — the ao-2c-era fear that gating would grind flow proved unfounded at n=1. Watch the false-positive rate as more high-risk slices pass through (P34 owns re-derivation from measured data).
