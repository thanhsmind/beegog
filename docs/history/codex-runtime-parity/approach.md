...
distributed through the existing root plugin/marketplace, while repo-local
Codex hooks become an explicit fallback generated by onboarding. Hook scripts
and helper business logic stay shared; only catalog matchers, payload adapters,
and event output encoding differ (per D1 and D2).
...
- Remove the committed, broken active `.codex/hooks.json`. Onboarding gains an
  explicit Codex repo-fallback route that renders from the Codex projection,
  resolves through the git root, preserves foreign hook entries, backs up
  changed config, and refuses when the bee plugin is enabled or its status is
  unknowable.
- Every Codex catalog command passes its source identity (`plugin` or `repo`).
  A per-repository selector is written atomically before migration behavior is
  enabled; wrappers execute only the selected source and reject an unknown or
  contradictory selector. This arbitration prevents double effects when an
  old fallback and a newly enabled global plugin temporarily coexist. The
  migration still removes the inactive bee fallback after plugin UAT so the
  final `/hooks` view has one configured source, not merely one effective one.
