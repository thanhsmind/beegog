...
- **W3** — pinned agent types: `.claude/agents/bee-*.md` with `model:` in frontmatter (`bee-gather`=sonnet, `bee-extract`=haiku, `bee-review`=opus); guard rejects catch-all `general-purpose` for bee dispatches. **This explicitly does not reduce cost** (CONTEXT.md) — no cell or report may claim it does. **Sync (AO10, binding):** `.claude/agents/` **must not** become a third `REPO_SKILL_TARGETS` entry — the three-version preflight would resolve it as `unknown` (no `bee-hive` *directory* under an agents root) and **refuse onboarding permanently and non-forceably on every host**. It ships through a separate flat managed-file sync with its own version marker. **Codex (AO11):** documented asymmetry, not parity — Codex has no per-agent model selection, so a pinned `model:` is a no-op file there.
  ...
  **Slice 3B design settled at planning (2026-07-17):**
  1. **Agent files are rendered from config at sync time, never hand-pinned** (AO5 — config is the authority; a static `model: sonnet` would drift the moment an owner reconfigures a tier). Source of truth: `skills/bee-hive/templates/agents/bee-{gather,extract,review}.md.tmpl` — frontmatter (`name`, `description`, `tools` read-only for gather/extract, `model: {{TIER_MODEL}}`) + a short body stating the worker contract (digest back, no writes for gather/extract). Onboarding renders `{{TIER_MODEL}}` from the host's `models.claude` slots (gather←generation, extract←extraction, review←review) into `<host>/.claude/agents/bee-*.md`.
  2. **Sync mechanism (AO10-safe):** a new flat managed-file step inside `onboard_bee.mjs` — NOT `REPO_SKILL_TARGETS` — same class as the settings-json hook merge: byte-render, write, and record `agents_sync: {version, files, rendered_from}` in `.bee/onboarding.json` as its own version marker. Cli-shaped or null slot → that agent file is **skipped and removed if present** (an agent type must name a real model); recorded in the sync record. **Per today's learning (Addendum 2): every generator inventory is taught in the same cell** — installers `VENDORED_*`/e2e expectations, `plugin_distribution.mjs` if it inventories agents, `test_onboard_bee.mjs` rows.
  3. **Guard rule (mechanical definition of "bee dispatch"):** in `bee-model-guard`, a dispatch that carries the anchored `[bee-tier: <tier>]` marker (the bee transport IS the signal) **and** `subagent_type: "general-purpose"` is denied: the FIX names the pinned type for that tier (`bee-gather`/`bee-extract`/`bee-review`) and the runtime read-only type (`Explore`) for read-only gathers. Marker + `Explore` (or any non-general-purpose type) stays allowed (compounding D1 analysts unaffected). Model-param-only dispatches (no marker) are untouched — they already passed membership; ceiling-marker dispatches name no pinned type (ceiling = session model, no agent file) and general-purpose stays **allowed** for `[bee-tier: ceiling]` (the one tier with no pinned agent). Config drift: `validateModelsConfig` gains an advisory problem code when a rendered agent file's `model:` no longer equals the configured tier model (surface at `bee status`/`config validate`, refusal not needed — the guard's marker+param equality already protects the dispatch itself).
  4. **Prose surfaces that instruct general-purpose spawning for bee work** update in the same slice: `bee-swarming/SKILL.md` §3 + `references/swarming-reference.md` worker template (spawn tier-matched pinned type when it exists; general default only for ceiling), `bee-validating` checker dispatch note. `bee-compounding` already mandates `Explore` — unchanged.
  5. **Codex (AO11):** `.agents/` gets NO agent files; the asymmetry is documented in the sync record and swarming reference (Codex enforces tiers as read budgets — shipped behavior).

  ##### Slice 3B cells (current slice)

  | Cell | Lane | Files bounded to | Verify command |
  |---|---|---|---|
  | `ao-3b-1` | standard | `skills/bee-hive/templates/agents/*.tmpl` (new), `skills/bee-hive/scripts/onboard_bee.mjs` (+`.claude`/`.agents` mirrors), `skills/bee-hive/scripts/test_onboard_bee.mjs` (+mirrors), `scripts/test_installers_e2e.mjs`, `skills/bee-hive/scripts/plugin_distribution.mjs` (+mirrors, if inventoried), `.claude/agents/bee-*.md` (rendered via self-onboard), `.bee/onboarding.json`, release manifest | `node skills/bee-hive/scripts/test_onboard_bee.mjs && node scripts/test_installers_e2e.mjs --installer bash && node skills/bee-hive/scripts/test_plugin_distribution.mjs && node scripts/release_manifest.mjs --check` |
  | `ao-3b-2` | standard | `hooks/bee-model-guard.mjs` + mirror, `hooks/test_model_guard.mjs`, `skills/bee-hive/templates/lib/state.mjs` + mirror (drift advisory), `scripts/test_config_validate.mjs`, `skills/bee-swarming/SKILL.md` + `references/swarming-reference.md`, `skills/bee-validating/SKILL.md` (+`.claude`/`.agents` mirrors), release manifest — deps: `ao-3b-1` (the FIX names types that must exist) | `node hooks/test_model_guard.mjs && node hooks/test_write_guard.mjs && node hooks/test_hook_contracts.mjs && node scripts/test_config_validate.mjs && node skills/bee-hive/templates/tests/test_lib.mjs && node scripts/test_lib_mirror.mjs && node scripts/release_manifest.mjs --check` |
