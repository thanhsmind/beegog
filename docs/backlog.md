...
| P7 | Keep the strong model scarce, measurably | `tier` field on cells + ceiling-share warning in bee_status/preamble; enforce scarcity instead of trusting prose (deferred from decision 0012) | proposed | — |
| P8 | Advisor pattern (cheap main loop, ceiling on demand) is first-class | Run the session on `generation`; a cross-model escalation gate consults `ceiling` only at Gate 2/3 or `[BLOCKED]` (deferred from decision 0012) | proposed | — |
