...
5. **Description = trigger conditions only** ("Use when…"), third person, ≤1024 chars, never a workflow summary — a summary makes agents follow the description and skip the body. Include symptoms and error keywords for discovery.
...
- **Description trigger:** "Use when a feature request needs user-facing decisions captured in history/<feature>/CONTEXT.md before planning. Clarifies fuzzy scope without implementation research or cell creation."
- **Dependencies:** none.
...
- **Description trigger:** "Use when reviewing completes or work is intentionally abandoned. Extracts durable patterns, decisions, and failures into history/learnings and the decision log, promoting only critical reusable lessons."
- **Dependencies:** none.
