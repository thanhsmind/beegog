...
- **Approval happens on the brief, but the brief is never the sole change site.** When the human comments on `implement-plan.md` ("change decision X", "add XLSX"), the change flows into the truth artifacts first — `bee-planning` revises `plan.md`; a locked decision is superseded via `node .bee/bin/bee_decisions.mjs supersede` and `CONTEXT.md` updated — THEN the brief re-renders so all documents agree. Hand-editing the brief alone creates a render that disagrees with its own source.
