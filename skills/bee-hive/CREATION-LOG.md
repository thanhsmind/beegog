...
- **State layer surfacing** (decision 0001): the session scout notes `docs/specs/` when present and mandates the per-area reading order spec → decisions → history; the scout-contract table adds the touched area's spec to "always read" in every lane, and `docs/specs/reading-map.md` is the first stop for "where does X live". Written by compounding, guarded by grooming; hive only reads.
