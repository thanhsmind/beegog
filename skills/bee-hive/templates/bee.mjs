...
// ─── response emission (manifest_changed only appears when true, so steady-
// state output stays byte-identical to the original CLIs — parity, D5) ─────

function emit({ result, text, exitCode = 0 }, useJson, drift) {
  if (drift && drift.manifest_changed) {
    if (useJson) {
      process.stdout.write(
        `${JSON.stringify({ manifest_changed: true, manifest_changed_hint: drift.hint, result }, null, 2)}\n`,
      );
    } else {
      process.stdout.write(`manifest_changed: true — ${drift.hint}\n${text}\n`);
    }
  } else if (useJson) {
