...
7. **Optional discovery:** `.bee/bin/bee.mjs` is the single dispatcher covering all 9 command groups (`status`, `cells`, `reservations`, `decisions`, `state`, `backlog`, `capture`, `reviews`, `feedback`) — every `bee_*.mjs` helper is a thin shim over it, so `bee_cells.mjs <verb>` and `bee.mjs cells <verb>` are the same implementation with byte-identical output. Run `node .bee/bin/bee.mjs --help --json` any time to see the full command surface as a Claude-Code tool-schema-shaped manifest (`{name, invoke, description, parameters, examples, deprecated}`). This is a discovery aid available on request, not a mandatory every-session call — an MCP server wrapper and a mandatory per-session discovery step were both considered and explicitly deferred (no such mandatory mechanism existed before this, so nothing here replaces one). The helper invocations keep working unchanged, exactly as in steps 1-6.
...
.bee/
  ...
  bin/                <- bee.mjs (unified dispatcher, all 9 command groups) + the 9 bee_*.mjs shims over it
