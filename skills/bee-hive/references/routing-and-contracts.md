...
### Verify Ladder (D4, `e54878b1`, superseded by ci-owned-verify D6)

A cell's `verify` field is always its **targeted** suite (seconds), never the full configured `commands.verify` chain. The four-milestone ladder above is retired: no local full run is ever a workflow obligation. The dev loop's own broader check is `commands.test` (the impacted run, `run_verify.mjs --impacted` / `--impacted-from-git`), authored from the impact registry (`impact_registry.mjs --query`) — wave close (once, by the orchestrator, per `bee-swarming/SKILL.md`) and worktree merge both run it instead of the full chain. The full `commands.verify` chain is CI-owned: it runs on the project's own CI cadence (push, nightly, or scheduled — the host workflow decides) and auto-files a `verify-red` issue when red; session finish and release gates still invoke it directly, unchanged (`AGENTS.md`). Judges and reviewers verify against the diff and `must_haves`, never by running the full chain as part of a verdict.
