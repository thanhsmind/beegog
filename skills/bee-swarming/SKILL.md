...
4. **Resolve each cell's model tier.** Use the cell's `tier` field (planning assigns it; default `generation`, `ceiling` kept scarce), and resolve it to a model from `.bee/config.json` `models` (runtime-keyed; decision 0012): `extraction` = cheapest capable (retrieval, mechanical edits), `generation` = mid (implementation, test writing), `ceiling` = the orchestrator's own model, **kept scarce** (integration, architecture, final review only — the cost lever is that ceiling touches few dispatches). On Claude Code set the Agent `model` param to `config.models.claude[tier]`; where the runtime cannot select per-agent models (Codex today, `null` tiers), state the tier in the prompt and enforce it as a read budget + output cap. Resolver: `modelForTier(root, tier, runtime)`. If `bee_status` flags ceiling scarcity, re-tier routine cells down before spawning.
