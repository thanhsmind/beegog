...
4. **Judge each cell's model tier at dispatch** — you (the orchestrator) assess the task in front of you and pick the fitting tier; it is NOT fixed by planning (a planning `tier` is at most a hint you may override; decision 0016). Rubric from the cell's lane + action + must_haves + files:
   ...
   Record the choice so scarcity stays measurable: `node .bee/bin/bee_cells.mjs tier --id <id> --tier <tier>`. Then resolve to a model with `modelForTier(root, tier, runtime)`: only the cheaper tiers are configured in `.bee/config.json` `models` (`extraction`/`generation`, runtime-keyed; decisions 0012/0015); **`ceiling` returns null → omit the Agent `model` param so the worker inherits the session model** (the strongest model in play). Where the runtime cannot select per-agent models (Codex, `null` tiers), state the tier in the prompt as a read budget + output cap. Keep `ceiling` scarce — if `bee_status` flags ceiling scarcity, re-judge routine cells downward before spawning.
