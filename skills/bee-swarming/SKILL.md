...
4. **Resolve each cell's model tier.** Use the cell's `tier` field (planning assigns it; default `generation`). Only the cheaper tiers are configured in `.bee/config.json` `models` (runtime-keyed; decisions 0012/0015): `extraction` = cheapest capable (retrieval, mechanical edits), `generation` = mid (implementation, test writing). **`ceiling` is never configured — it is the session/orchestrator model itself**, so a ceiling cell runs on the strongest model in play; keep it scarce (integration, architecture, final review only). On Claude Code set the Agent `model` param to `modelForTier(root, tier, runtime)`: for `generation`/`extraction` that is the configured alias; for `ceiling` it returns **null → omit the `model` param so the worker inherits the session model**. Where the runtime cannot select per-agent models (Codex today, `null` tiers), state the tier in the prompt and enforce it as a read budget + output cap. If `bee_status` flags ceiling scarcity, re-tier routine cells down before spawning.
