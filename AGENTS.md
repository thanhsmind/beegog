...
9. **Lanes scale ceremony, never memory — zero exceptions.** A settled rule, behavior, or value gets captured the moment it settles — into `docs/knowledge/` when the repo has a bundle, else `docs/specs/` — tiny lanes included, whether or not the lane produced a `plan.md` (D3/D4) — never left in the chat. This covers every flow shape, the docs lane and non-cell quick work included: no shape is exempt. Detecting settlement is the agent's job, every turn, unprompted; the user should never have to ask. Every task close carries either a decision-log/capture-stub line or an explicit "nothing settled" statement — a close with neither is not a close. Full trigger, lane-scaling, and detection discipline: `bee-scribing` skill.
