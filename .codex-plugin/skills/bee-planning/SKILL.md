...
Classify before you bootstrap. From the **request text plus at most 2 targeted file reads**, run a cheap **intake classification** — the Mode Gate below — to pick the lane. Only then do the lane-scaled bootstrap (§2). Tiny work must not pay full context reads before it knows it is tiny; this deliberately supersedes the old unconditional full-bootstrap-first ordering (D8). AGENTS.md startup step 6 (read the critical patterns before planning) still applies — and since f4-5 it is itself bundle-branched: `docs/knowledge/index.md`'s `## Critical patterns` section with a bundle, `docs/history/learnings/critical-patterns.md` without.
...
- **tiny:** the targeted reads only (the ≤2 from intake), plus the mandatory critical-patterns digest already in the preamble. When the active feature has a `bee.work-item` concept in `docs/knowledge/`, also run `node .bee/bin/bee.mjs knowledge context --work <feature> --budget 20000` and read the manifest's files — the same condition the standard/high-risk bootstrap uses below, just scaled to a tiny read.
- **small:** bounded bootstrap — `CONTEXT.md` if one exists + recent decisions (`node .bee/bin/bee.mjs decisions active --recent 3`). When the active feature has a `bee.work-item` concept in `docs/knowledge/`, also run `node .bee/bin/bee.mjs knowledge context --work <feature> --budget 20000` and read the manifest's files — same condition as standard/high-risk.
